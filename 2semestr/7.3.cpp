...
vect *reshitObshchuyu()
{
    ...
    if (ch == 1)
        X->zapolnitVruchnuyu("X");
    else
        X->zapolnitSluchayno("X");

    cout << "\nМассив Y:\n";
    ch = vvediChislo("1 - ручной ввод, 2 - случайно: ", 1, 2);
    if (ch == 1)
        Y.zapolnitVruchnuyu("Y");
    else
        Y.zapolnitSluchayno("Y");
...
void reshitIndividualnuyu(vect *gotovyMas = nullptr)
{
    ...
    if (nasNovyy)
    {
        ...
        int ch = vvediChislo("1 - ручной ввод, 2 - случайно: ", 1, 2);
        if (ch == 1)
            mas->zapolnitVruchnuyu("mas");
        else
            mas->zapolnitSluchayno("mas");
        mas->print("mas");
    }

    cout << "\nРезультат:\n";
    double maxMod = mas->get(0).modul();
    for (int i = 1; i < mas->getSize(); i++)
        if (mas->get(i).modul() > maxMod)
            maxMod = mas->get(i).modul();

    cout << fixed << setprecision(4);
    cout << "Максимальный модуль: " << maxMod << "\n";
    cout << "индексы:\n";
    int znac = 0;
    for (int i = 0; i < mas->getSize(); i++){
        if (fabs(mas->get(i).modul() - maxMod) < 1e-9)
        {
            cout << i << ", " ;
        }
        znac = i;
